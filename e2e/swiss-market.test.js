...
describe('Swiss Market Features', () => {
  ...
  describe('Swiss Timezone Handling', () => {
    ...
    it('should respect Swiss business hours (8:00-18:00)', async () => {
      await element(by.id('create-auction-button')).tap()

      // Duration input for business hours validation
      await element(by.id('duration-input')).clearText()
      await element(by.id('duration-input')).typeText('480') // 8 hours

      // Should validate against Swiss business hours constraints
      // This would need backend validation, but UI should show appropriate feedback
      await element(by.id('create-auction-submit')).tap()

      // Should either succeed or show appropriate business hours message
      await waitFor(element(by.text('Auction created')).or(by.text(/business hours/)))
        .toBeVisible()
        .withTimeout(5000)
    })
  })
